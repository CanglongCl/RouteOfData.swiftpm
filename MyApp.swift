...
private func insertRouteHealthExp(container: ModelContainer) {
    let route = Route(name: "Health Expectation", url: Bundle.main.url(forResource: "healthexp", withExtension: "csv")!, remark: "This dataset displays the life expectancy and health spending in countries across years. Click the starred nodes to see beautiful charts generated by Swift Chart. Click top-left bottom to go to the next route. ")
    container.mainContext.insert(route)
...
private func insertRouteApp(container: ModelContainer) {
    let route = Route(name: "App Revenue", url: Bundle.main.url(forResource: "app_financial_report", withExtension: "csv")!, remark: "This dataset shows the revenue of my apps in October 2023, exported from App Store Connect. I explore where these revenues come from and what proportion they represent.")
    container.mainContext.insert(route)
